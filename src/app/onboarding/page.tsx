...
export default function OnboardingPage() {
  ...
  const [selections, setSelections] = useState<
    { chosenCrop: string; customCrop: string; seedingDate: string }[]
  >([{ chosenCrop: "", customCrop: "", seedingDate: todayISO() }]);
  ...
  const setNumFarms = (n: number) => {
    ...
    setSelections((prev) => {
      const copy = [...prev];
      while (copy.length < next) copy.push({ chosenCrop: "", customCrop: "", seedingDate: todayISO() });
  ...
  const updateSelection = (idx: number, patch: Partial<(typeof selections)[number]>) => {
  ...
  const handleFetchAI = async () => {
    ...
    try {
      ...
      // auto-select first recommendation for each farm
      setSelections((prev) =>
        prev.map((s, idx) => ({
          ...s,
          chosenCrop: data.farms?.[idx]?.[0]?.cropName ?? s.chosenCrop,
          customCrop: "",
          seedingDate: s.seedingDate || todayISO(),
        }))
      );
  ...
  const canContinueDashboard = useMemo(() => {
    ...
    for (let i = 0; i < form.numFarms; i++) {
      ...
      if (!sel) return false;
      const finalCrop = sel.customCrop.trim() || sel.chosenCrop.trim();
        ...
        <div className="relative flex items-center justify-center px-6 py-12">
          ...
          <div className="w-full max-w-2xl relative z-10">
            <div className="mb-5 flex items-center justify-between">
              ...
              <span className="font-mono text-[11px] font-bold text-af-muted">
                Step {step} of 3
              </span>
            </div>
