      ...
      <div className="flex items-center gap-4 mt-3">
        ...
        <div>
          <div className="flex items-start">
            <span className="font-mono text-5xl font-extrabold text-af-ink leading-none">
              {current.temp}
            </span>
            <span className="text-xl font-bold text-af-muted mt-1">°C</span>
          </div>
      ...
      <div className="mt-6 pt-5 border-t border-af-border">
        <div className="grid grid-cols-7 gap-1.5">
          {daily.map((d, i) => (
            ...
            >
              <span className="text-[10px] font-bold text-af-muted uppercase">
                {i === 0 ? "Today" : d.weekday}
              </span>
              <span className="text-lg leading-none">{d.icon}</span>
              <span className="font-mono text-[11px] font-bold text-af-ink">{d.tMax}°</span>
              <span className="font-mono text-[10px] text-af-muted">{d.tMin}°</span>
              {d.precipProb >= 30 && (
                <span className="font-mono text-[9px] font-bold text-af-ai">{d.precipProb}%</span>
              )}
            </div>
          ))}
        </div>
...
function Stat({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="rounded-xl bg-af-bg border border-af-border px-3 py-2.5">
      ...
      <div className="mt-1.5 font-mono text-sm font-bold text-af-ink">{value}</div>
